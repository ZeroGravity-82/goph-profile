...
- перехватывает `X-Accel-Redirect` и отдает дефолтную аватарку как статический файл.
...
Операции воркера рассчитаны на повторную доставку сообщений: некорректные сообщения отбрасываются, а временные ошибки возвращают сообщение в очередь. Полная политика повторных попыток с задержкой, лимитом попыток и DLQ остается возможным улучшением.
...
Машиночитаемое описание запросов, ответов и ошибок находится в спецификации
[OpenAPI 3.0](docs/openapi.yaml). Её можно открыть в Swagger Editor, Swagger UI или другом совместимом инструменте.

Реализованные API-ручки:
...
Параметр `format` не выполняет конвертацию. Если он задан, запрошенный формат должен совпадать с фактическим форматом возвращаемого объекта. Если формат не совпадает с выбранным объектом, API возвращает `400 Bad Request`. Оригинал сохраняет исходный MIME-тип, миниатюры отдаются как PNG.
...
Полноценная аутентификация не реализована.

Текущая модель:

- TLS отвечает за безопасность соединения между клиентами, включая SPA, и сервером;
- `X-User-ID` считается доверенным заголовком;
...
Стандартная PNG-аватарка встроена в бинарник HTTP-сервера, поэтому она возвращается и при прямом доступе через Ingress.
При локальном запуске Nginx перехватывает дополнительный заголовок `X-Accel-Redirect` и отдает тот же файл из каталога
...
### Circuit breaker для MinIO

Операции чтения, записи и удаления объектов защищены с помощью circuit breaker. После пяти последовательных ошибок
соединения, тайм-аутов или ответов MinIO с кодом `5xx` circuit breaker переходит в открытое состояние на 30 секунд. В это
время новые операции сразу завершаются ошибкой без обращения к MinIO. После ожидания пропускается один пробный запрос: при
успехе обычная работа возобновляется, а при ошибке circuit breaker снова переходит в открытое состояние.

Каждая операция чтения, записи или удаления объекта в MinIO ограничена 30 секундами. При превышении этого времени операция
завершается ошибкой, которая учитывается circuit breaker как отказ хранилища.

Ответы MinIO с кодами `4xx` считаются успешными с точки зрения circuit breaker, потому что подтверждают доступность
хранилища. Отмена контекста клиентом также не считается отказом MinIO. Проверка `Ping` выполняется в обход circuit breaker,
чтобы проверка готовности продолжала обнаруживать восстановление хранилища.

Для PostgreSQL отдельный circuit breaker не добавлен: восстановлением соединений управляет `pgxpool`. Для RabbitMQ
отдельный circuit breaker на данном этапе тоже не добавлен. Сначала RabbitMQ-паблишера нужно научить пересоздавать соединение
и канал после обрыва связи. Иначе разрешенный после паузы пробный запрос будет выполняться через закрытый канал и не сможет
подтвердить восстановление RabbitMQ.
