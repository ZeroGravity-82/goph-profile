...
Ответы MinIO с кодами `4xx` не влияют на состояние circuit breaker, потому что подтверждают доступность хранилища. Отмена
контекста клиентом также не считается отказом MinIO. Проверка `Ping` выполняется в обход circuit breaker, чтобы
проверка готовности продолжала обнаруживать восстановление хранилища.
